...
> Status: **🎉 ALL 9 PHASES COMPLETE.** DevDocs AI is feature-complete: grounded RAG chat, semantic search, GitHub ingestion, four ReAct agents, and a production-quality pass (rate limiting, usage tracking, security hardening). Decisions D1–D3 resolved (§15). **A live run needs one free Gemini key** (`Gemini:ApiKey`) + Ollama running; tests use fakes and pass without either.
> Done so far:
> - **Phase 1** (Foundation).
...
>
> - **Phase 8** (Agents & tools): four built-in agents (Code Explorer, Documentation Generator, Bug Analysis, Architecture Analyst) that answer by calling tools over indexed content via a **ReAct loop** — no change to the chat port (tools ride on plain text completion). `AgentRun` aggregate (owns `ToolExecution` trace; status reuses `ProcessingStatus`) + `ToolExecutionStatus` enum; `AddAgentRuns` migration. `ReActParser` (tolerant: extracts first balanced JSON, reads `action`/`final_answer`). `IAgentTool` + `ToolRegistry` (allow-list per agent) with 3 tools: **SearchProject** (reuses `IRetrievalService`), **ReadFile** (`GetByPathAsync` → storage, line-numbered, capped), **GetProjectStructure**. `AgentCatalog` = built-in definitions (persona prompt + allowed tools; Bug Analysis separates *Evidence from the codebase* vs *Hypotheses*). `AgentService` runs a bounded loop (`AgentOptions.MaxIterations=8`), catches tool errors → recorded + fed back, persists run + trace; synchronous, returns answer + full trace. `AgentsController` `/api/v1/projects/{id}/agents` (list / `{type}/run` / runs history). **Frontend**: Agents tab (Code Explorer / Bug Analysis / Architecture Analyst picker → Markdown answer + expandable tool trace) and Documentation tab (Doc Generator); `agent-trace.tsx`. Integration tests use an **agent-aware fake chat provider** (SearchProject action → observation → final_answer) — network-free. Design + plan: `docs/superpowers/{specs,plans}/2026-07-28-agents-and-tools*`.
>
> - **Phase 9** (Production quality — final): **rate limiting** (ASP.NET Core built-in; `auth` 10/min, `ai` 20/min, global 100/min, partitioned by user id else IP; 429 ProblemDetails) on auth + AI controllers via `[EnableRateLimiting]`. **Usage tracking**: `UsageRecord`(userId, projectId, kind, tokensIn/out) + `UsageKind` + `AddUsageRecords` migration; best-effort `IUsageRecorder` (never breaks a request) recording Ask/Chat/AgentRun with `TokenEstimator` (~len/4) estimates; `UsageService` summary + `UsageController` `GET …/usage` + Overview usage card. **Security pass**: `SecurityHeadersMiddleware` (nosniff, DENY, no-referrer, CSP), `RequestContextMiddleware` (per-request RequestId + UserId into Serilog `LogContext`), and a unit test locking in the exception handler's generic-500 (no detail leak). **README** rewritten as a portfolio front door (features, mermaid architecture, 9-phase summary, decisions, getting-started). Existing auth/logging/error-handling/Docker/CI were already in place. Design + plan: `docs/superpowers/{specs,plans}/2026-07-28-production-quality*`.
>
> **Final test count: 182 passing** (139 unit + 43 Testcontainers integration), backend build 0 warnings / 0 errors. Frontend typecheck + lint + production build clean.
>
...
>
> **Project complete.** All nine phases delivered, each a tested, mergeable slice. Possible future work (beyond the spec's scope): parse exact provider token counts for usage; a pluggable-model reranker; agent streaming; private-repo support; a deployment target (Phase 9's Docker images make the platform interchangeable). Each of the nine phases has its design + plan under `docs/superpowers/`.
