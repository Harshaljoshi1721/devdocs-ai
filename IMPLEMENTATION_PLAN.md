...
> Status: **Phase 6 (AI Chat) complete** — next up is Phase 7 (GitHub integration). Decisions D1–D3 resolved (§15). **A live run needs one free Gemini key** (`Gemini:ApiKey`) + Ollama running; tests use fakes and pass without either.
> Done so far:
> - **Phase 1** (Foundation).
...
>
> **Test count: 105 passing** (78 unit + 27 Testcontainers integration), backend build 0 warnings / 0 errors. Frontend typecheck + lint + production build clean.
>
...
>
> **Next session → Phase 7 (GitHub integration):** `IGitHubRepositoryClient`, URL validation, clone/fetch, `.gitignore` + filter respect, repo ingestion reusing the Phase 3–5 pipeline, connection status. *Milestone: connect a public repo and query it.* (Phase 6 shipped conversations + streaming chat + the chat/search/upload UI — see the Phase 6 bullet above.)
